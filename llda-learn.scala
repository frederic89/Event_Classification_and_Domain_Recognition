...
val source = CSVFile("09 待Labeled-LDA运算.csv") ~> IDColumn(1);  // 序号列
...
val text = {
  ...
  TermMinimumDocumentCountFilter(2) ~>   // 【已改成最佳2】filter terms in <4 docs
  TermDynamicStopListFilter(40) ~>       // 【已改成最佳40】filter out 30 most common terms
  DocumentMinimumLengthFilter(3)         // 【已改成最佳3】take only docs with >=5 terms
}
